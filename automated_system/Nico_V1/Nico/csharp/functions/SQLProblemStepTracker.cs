...
namespace Nico.csharp.functions
{
    public class SQLProblemStepTracker
    {
        ...
        public static void UpdateProbStep(string userid, int sessionid, int problemid, int stepid, int problemimgkey, int stepanswerkey, int newanswer, int numautoresponses)
        {
            try
            {
                ...
                sql = "UPDATE NicoDB.dbo.Problem_Step_Tracker SET ProblemID=@ProblemID, StepID = @StepID, ProblemImgKey = @ProblemImgKey, StepAnswerKey = @StepAnswerKey, NewAnswer = @NewAnswer, NumAutoResponses = @NumAutoResponses WHERE NicoDB.dbo.Problem_Step_Tracker.UserID = @UserID and NicoDB.dbo.Problem_Step_Tracker.SessionID = @SessionID";
        ...
        public static void WriteNewProbStep(string userid, int sessionid, int problemid, int stepid, int problemimgkey, int stepanswerkey, int newanswer, int numautoresponses)
        {
            try
            {
                ...
                connectionString = ConfigurationManager.ConnectionStrings["NicoDB"].ConnectionString;
                connection = new SqlConnection(connectionString);
                sql = "Insert into NicoDB.dbo.Problem_Step_Tracker Values(UserID = @UserID, SessionID = @SessionID, ProblemID = @ProblemID, StepID = @StepID, ProblemImgKey = @ProblemImgKey, StepAnswerKey = @StepAnswerKey, NewAnswer = @NewAnswer, NumAutoResponses = @NumAutoResponses";
                SqlCommand cmd = new SqlCommand(sql, connection);
